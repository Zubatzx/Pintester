        ...
        @else
            <nav class="navbar navbar-expand beta-menu navbar-dropdown align-items-center navbar-fixed-top navbar-toggleable-sm">
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <div class="navbar-buttons mbr-section-btn"><a class="btn btn-sm btn-primary display-4" href="{{ route('myProfile', ['id' => session()->get('userID')]) }}"><span class="mbr-iconfont mbr-iconfont-btn"><img class="rounded-circle profilePicture" src="{{asset('assets/images/users/'.session()->get('profilePicture'))}}" alt="{{ session()->get('name') }}" /></span>{{ session()->get('name') }}</a>
